...
public class Main {
    ...
    static       String PROVIDER_URL = "iiop://";
    ...
    public TextbookSearchServer connect(String ip){
        try{
            ...
            Main.PROVIDER_URL = Main.PROVIDER_URL + ip;

            //Create initial context
            Properties props = new Properties();
            props.put(Main.FACTORY, Main.FACTORY_NAME);
            props.put(Main.PROVIDER, Main.PROVIDER_URL);

            InitialContext ic = new InitialContext(props);

            //Obtain reference from interface
            TextbookSearchServer tss;
            tss = (TextbookSearchServer) PortableRemoteObject.narrow(ic.lookup("TextbookServer"), TextbookSearchServer.class);
            return tss;
        } catch(Exception ex){
